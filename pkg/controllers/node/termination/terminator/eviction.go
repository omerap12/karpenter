...
func (q *Queue) Evict(ctx context.Context, key QueueKey) bool {
	...
	evictionReasonMessage := evictionReason(ctx, key, q.kubeClient)
	q.recorder.Publish(terminatorevents.EvictPod(&corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: key.Name, Namespace: key.Namespace}}, evictionReasonMessage))
	PodsDrainedTotal.Inc(map[string]string{ReasonLabel: evictionReasonMessage})
	return true
}
